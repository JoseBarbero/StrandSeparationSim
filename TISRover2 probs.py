...
def tisrover():
    ...
    model.add(Conv1D(filters=200, kernel_size=3, activation='relu', input_shape=(200, 36)))
...
if __name__ == "__main__":
    ...
    X_train_probs = np.moveaxis(X_train[:,:,:,0], 1, 2)
    X_val_probs = np.moveaxis(X_val[:,:,:,0], 1, 2)
