...
def tisrover3xtisrover3():
    ...
    probs = Sequential()
    probs.add(Conv2D(filters=50, kernel_size=(2, 2), activation='relu', input_shape=(200,28,5)))
...
if __name__ == "__main__":
    ...
    X_train_cnn = np.moveaxis(X_train[:,:,:,:5], 0, 1)
    X_val_tisrover = X_val[:,1,:,5:13]
    X_val_cnn = np.moveaxis(X_val[:,:,:,:5], 0, 1)
    X_test_tisrover = X_test[:,1,:,5:13]
    X_test_cnn = np.moveaxis(X_test[:,:,:,:5], 0 ,1)
