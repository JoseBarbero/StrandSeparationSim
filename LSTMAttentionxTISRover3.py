...
def lstmattxtisrover3():
    ...
    x = tf.keras.layers.Conv2D(filters=50, kernel_size=(2, 2), activation='relu')
    x = tf.keras.layers.Dropout(0.2)
    x = tf.keras.layers.Conv2D(filters=62, kernel_size=(2, 2), activation='relu')
    x = tf.keras.layers.MaxPooling2D((2,2))
    x = tf.keras.layers.Dropout(0.2)
    x = tf.keras.layers.Conv2D(filters=75, kernel_size=(2, 2), activation='relu')
    x = tf.keras.layers.MaxPooling2D((2,2))
    x = tf.keras.layers.Dropout(0.2)
    x = tf.keras.layers.Conv2D(filters=87, kernel_size=(2, 2), activation='relu')
    x = tf.keras.layers.MaxPooling2D((2,2))
    x = tf.keras.layers.Dropout(0.2)
    x = tf.keras.layers.Conv1D(filters=100, kernel_size=2, activation='relu')
    x = tf.keras.layers.MaxPooling2D((2,2))
    x = tf.keras.layers.Dropout(0.2)
    x = tf.keras.layers.Flatten()
    x = tf.keras.layers.Dense(128, activation='relu')
    x = tf.keras.layers.Dropout(0.5)
    cnn_output = tf.keras.layers.Flatten()
