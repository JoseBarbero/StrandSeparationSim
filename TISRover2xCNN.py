...
def tisroverxcnn():
    ...
    tisrover.add(Conv1D(filters=200, kernel_size=3, activation='relu', input_shape=(200, 8)))
    tisrover.add(MaxPooling1D(6))
    tisrover.add(Dropout(0.2))

    tisrover.add(Conv1D(filters=200, kernel_size=3, activation='relu'))
    tisrover.add(MaxPooling1D(6))
    tisrover.add(Dropout(0.2))

    tisrover.add(Flatten())
    tisrover.add(Dense(64))
    tisrover.add(Dropout(0.6))
    tisrover.add(Flatten())
