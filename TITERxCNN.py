...
def titerxcnn():
    ...
    titer = Sequential() 
    titer.add(Conv1D(filters=128, kernel_size=3, activation='relu', input_shape=(200, 8)))
    titer.add(MaxPooling1D(3))
    titer.add(Dropout(0.25))
    titer.add(LSTM(256, return_sequences=True, go_backwards=False))
    titer.add(Dropout(0.8))
    titer.add(Flatten())
    ...
    with open(log_file, 'w') as f:
        with redirect_stdout(f):
            ...
            history = model.fit([X_train_titer, X_train_cnn], y_train,
                                ...
                                epochs=50,
                                verbose=True,
                                validation_data=([X_val_titer, X_val_cnn], y_val),
